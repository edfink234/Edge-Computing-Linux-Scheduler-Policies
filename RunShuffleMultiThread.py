'''
Driver Program to run the ShuffleMultiThread executable 10 times per
scheduling policy, and to store the results in a csv file:
RunShuffleAllTimes.txt
'''
...
for (policy,min_priority) in zip(policies,min_priorities):
    ...
    for i in range(10):
        proc = subprocess.Popen(["./ShuffleMultiThread"], stdout=subprocess.PIPE, shell=True)
        pid = os.popen("sudo pidof -s ./ShuffleMultiThread").read()
...
data.to_csv("RunShuffleAllTimes.txt")
