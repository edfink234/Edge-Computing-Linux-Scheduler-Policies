...
for (policy,min_priority) in zip(policies,min_priorities):
    ...
    for i in range(10):
        proc = subprocess.Popen(["./PermuteMultiThread"], stdout=subprocess.PIPE, shell=True)
        pid = os.popen("sudo pidof -s ./PermuteMultiThread").read()
...
data.to_csv("RunPermuteAllTimes.txt")
