...
#os.system('scp magix@detlab-101.prisma.uni-mainz.de:"/home/magix/RunShuffleAllTimes.txt" ./')
hists = {}
policies = ('--batch','--fifo','--idle','--other','--rr')

data = pd.read_csv("RunShuffleAllTimes.txt",index_col=False)
...
plt.title("Thread Run-Time Averages Random Token Shuffle")
plt.savefig("Thread_Averages_Random_Shuffle.png",dpi=5*96)
plt.close()
...
plt.xticks([],[])
plt.ylabel("Time (Nano-seconds)")
plt.title("Total Run-Time Averages Random Token Shuffle")
plt.savefig("Total_Averages_Random_Shuffle.png",dpi=5*96)
plt.close()
...
for i in hists:
    ...
    plt.legend()
    if i == 't':
        plt.title("Total Program Run Times Random Token Shuffle")
        plt.savefig(f"RunTimesRunShuffleTotal.png",dpi=5*96)
    else:
        plt.title(f"Thread {i} Run Times Random Token Shuffle")
        plt.savefig(f"RunTimesRunShuffleThread{i}.png",dpi=5*96)
    plt.close()
