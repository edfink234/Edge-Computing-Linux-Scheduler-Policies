...
os.system('scp magix@detlab-101.prisma.uni-mainz.de:"/home/magix/RunDescribeAllTimes.txt" ./')
hists = {}
policies = ('--batch','--fifo','--idle','--other','--rr')

data = pd.read_csv("RunDescribeAllTimes.txt",index_col=False)
...
plt.title("Thread Run-Time Averages Random Number Stats")
plt.savefig("Thread_Averages_Random_Number_Stats.png",dpi=5*96)
plt.close()
...
plt.xticks([],[])
plt.ylabel("Time (Nano-seconds)")
plt.title("Total Run-Time Averages Random Number Stats")
plt.savefig("Total_Averages_Random_Number_Stats.png",dpi=5*96)
plt.close()
